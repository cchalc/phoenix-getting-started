defmodule PhxLearn.ToDosTest do
  ...
  describe "todos" do
    ...
    test "create_to_do/1 with valid data creates a to_do" do
      valid_attrs = %{deadline: ~U[2024-12-17 06:14:00Z], done: true, name: "some name"}

      assert {:ok, %ToDo{} = to_do} = ToDos.create_to_do(valid_attrs)
      assert to_do.deadline == ~U[2024-12-17 06:14:00Z]
    ...
    test "update_to_do/2 with valid data updates the to_do" do
      to_do = to_do_fixture()
      update_attrs = %{deadline: ~U[2024-12-18 06:14:00Z], done: false, name: "some updated name"}

      assert {:ok, %ToDo{} = to_do} = ToDos.update_to_do(to_do, update_attrs)
      assert to_do.deadline == ~U[2024-12-18 06:14:00Z]
